...
func _node_id_at(local_point: Vector2) -> int:
	## Find the hex whose center is closest to `local_point`, within
	## a hex-radius tolerance. Used by click hit-testing.
	var best_id := -1
	var best_d := Tokens.HEX_R * 1.05
	for node_id in _hex_nodes.keys():
		var hex = _hex_nodes[node_id]
		var d := local_point.distance_to(hex.position)
		if d < best_d:
			best_d = d
			best_id = node_id
	return best_id
